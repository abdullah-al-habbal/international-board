...
final class AdminActionPerformed extends Notification
{
    ...
    private function getViewUrl(): string
    {
        ...
        return match (true) {
            array_key_exists('view', $pages) => $resource::getUrl('view', ['record' => $this->model->getKey()]),
            array_key_exists('edit', $pages) => $resource::getUrl('edit', ['record' => $this->model->getKey()]),
            default => $resource::getUrl(),
        };
    }
