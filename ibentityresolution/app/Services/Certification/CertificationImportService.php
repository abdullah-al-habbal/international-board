 ...
 *
 * The structural change from the previous version is that entity resolution is now
 * BATCHED rather than per-row.
 *
 * Before: every chunk job called warmUpHandlers(), which read the whole trainees
 * table into a PHP array before touching a single row. Two hundred chunk jobs meant
 * two hundred full table scans, and the cost grew with every import because the
 * table it scanned kept getting bigger. Worse, the array was per-process, so two
 * jobs meeting the same new trainee both inserted one.
 *
 * Now: rows are buffered in batches of 500. Each batch collects its distinct names,
 * resolves them in a handful of indexed queries, then maps every row from an
 * in-memory map. The unique index on name_key — not the array — is what guarantees
 * a name maps to one id, so parallel workers are safe.
 *
 * Query cost per 500-row batch is roughly constant: two SELECTs and at most one
 * write per entity type, plus one upsert for the certifications themselves.
 */
final class CertificationImportService
{
    ...
    private function flushBatch(array $buffer, array $headers, int $creatorId, array &$stats): void
    {
        ...
        // Countries first: a trainee's insert payload needs its country id.
        ...
        foreach ($extracted as $values) {
            ...
            // First non-null country wins for a brand-new trainee; later rows only
            // ever read the id back, they never rewrite it.
