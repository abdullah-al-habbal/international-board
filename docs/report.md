...
| Item                            | Resolution                                                                                   |
| ------------------------------- | -------------------------------------------------------------------------------------------- |
| Home `about_section` DB content | Shows `StaticPage` content excerpt when `about-us` page has content; lang checklist fallback |
| Centers country filter          | Dropdown in `_filters`; `CertifiedCenterService::getFilterCountries()`                       |


## Recommendations

1. Add feature tests for `web.blog.index` and trainer/center index pagination *(optional — skipped)*.
